...
<a class="navbar-brand" style="margin-top: 5px;color:white;" href="#">
  ...
  <ul class="navbar-nav design">
      ...
      <a class="nav-link dropdown-toggle design" href="video_index.php" id="navbardrop" data-toggle="dropdown">
      Video Song
      </a>
      <div class="dropdown-menu " style="background-color:RGB(150,52,125);">
        
        <a class="dropdown-item design" href="audio_index.php">Audio Song</a>
      </div>
    </li>
     <?php
     $query="SHOW COLUMNS FROM video_music";
     $result=mysqli_query($conn,$query);
     while($navname=mysqli_fetch_array($result)){
    if($navname[0]=='id'|| $navname[0]=='video' || $navname[0]=='image'|| $navname[0]=='name'){
    }else{
    ?>
     <li class="nav-item dropdown">
      ...
      <div class="dropdown-menu " style="background-color:RGB(150,52,125);">
      <?php
      $query1="SELECT $navname[0] FROM video_music GROUP BY $navname[0] HAVING COUNT($navname[0]) >1";
      $result1=mysqli_query($conn,$query1);
      while($navname1=mysqli_fetch_array($result1)){
     ?>
     <form method="POST" action="video_index.php">
